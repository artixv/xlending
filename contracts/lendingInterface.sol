...
contract lendingInterface {
        ...
        returns (uint[] memory availableAmount)
    {
        ...
        for (uint i = 0; i < assetLength; i++) {
            depositAndLendAmount = assetsDepositAndLendAmount(
    ...
    function licensedAssetPrice()
        public
        view
        returns (uint[] memory assetPrice)
    {
        return iLendingManager(lendingManager).licensedAssetPrice();
    }
    function licensedAssetOverview()
        external
        view
        returns (
            uint totalValueOfMortgagedAssets,
            uint totalValueOfLendedAssets
        )
    {
    ...
    function usersHealthFactorAndInterestEstimate(
        ...
        external
        view
        returns (uint userHealthFactor, uint[2] memory newInterest)
    {
        ...
        if (operator == 0) {
            _amountDeposit +=
                (amount * iSlcOracle(oracleAddr).getPrice(token)) /
                1 ether;
        } else if (operator == 1) {
            _amountDeposit -=
                (amount * iSlcOracle(oracleAddr).getPrice(token)) /
        ...
        if (iDepositOrLoanCoin(depositAndLend[0]).totalSupply() > 0) {
            if (operator == 0) {
                _amountDeposit += amount;
            } else if (operator == 1) {
                if(_amountDeposit > amount){
                    _amountDeposit -= amount;
                }else{
                    _amountDeposit = 0;
                }
            ...
            if (_amountDeposit > 0) {
                lendingRatio = (_amountLending * 10000) / _amountDeposit;
            }else{
                lendingRatio = 0;
            }
        } else {
    ...
    function viewUserLendableLimit(
        ...
        if (_userMode <= 1) {
            if (
                (_amountDeposit * 1 ether) / nomalFloorOfHealthFactor() >
                _amountLending
            ) {
                userLendableLimit =
            ...
            if (
                (_amountDeposit * 1 ether) / homogeneousFloorOfHealthFactor() >
                _amountLending
            ) {
                userLendableLimit =
                    (_amountDeposit * 1 ether) /
    ...
    function userProfile(
        ...
        if (tempRustFactor[0] == 0) {
            ...
            netApy =
                (int(tempRustFactor[3]) - int(tempRustFactor[4])) /
                fullWorth;
        }
    }
